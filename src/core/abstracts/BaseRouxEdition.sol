...
/**
 * @title base roux edition
 * @author roux
 * @custom:security-contact security@roux.app
 */
abstract contract BaseRouxEdition is IRouxEdition, ERC1155, ERC165, Initializable, OwnableRoles, ReentrancyGuard {
     ...
     * @return id token id
     *
     * @dev implementation for `add` is same for all RouxEdition child contracts, only permissions should differ
     */
    function _add(EditionData.AddParams calldata p) internal returns (uint256) {
        RouxEditionStorage storage $ = _storage();
     ...
     */
    function _setCollection(address collection, bool enable) internal {
        if (enable) {
     ...
     *
     * @dev underflow error will occur if parent uri is not set (should not be setting a parent if no uri)
     */
