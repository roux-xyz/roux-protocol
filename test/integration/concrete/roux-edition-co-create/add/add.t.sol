...
contract Add_RouxEditionCoCreate_Integration_Concrete_Test is BaseTest {
    /* -------------------------------------------- */
    /* setup                                       */
    /* -------------------------------------------- */
    ...
    function test__AddToken_ControllerDataIsSet() external {
        ...
        address fundsRecipient_ = controller.fundsRecipient(address(coCreateEdition), tokenId_);
        uint256 profitShare_ = controller.profitShare(address(coCreateEdition), tokenId_);
    ...
    function test__AddToken_RegistryDataIsSet() external {
        ...
        (address parentEdition_, uint256 parentTokenId_, uint256 idx) =
            registry.attribution(address(coCreateEdition), tokenId_);
    ...
    function test__AddToken_Fork_RegistryDataIsSet() external {
        (IRouxEdition forkEdition_, uint256 tokenId_) = _createFork(coCreateEdition, 1, users.creator_1);

        (address parentEdition_, uint256 parentTokenId_, uint256 idx) =
            registry.attribution(address(forkEdition_), tokenId_);

        assertEq(parentEdition_, address(coCreateEdition));
    ...
    function test__AddToken_Fork_2ndLevel_RegistryDataIsSet() external {
        (RouxEdition forkEdition_, uint256 tokenId_) = _createFork(coCreateEdition, 1, users.creator_1);
    ...
    function test__AddToken_MaxNumForks() external {
        RouxEdition[] memory coCreateEditions = new RouxEdition[](MAX_CHILDREN + 1);
        coCreateEditions[0] = coCreateEdition;

        for (uint256 i = 1; i <= MAX_CHILDREN; i++) {
            (coCreateEditions[i],) = _createFork(coCreateEditions[i - 1], 1, users.creator_1);

            (,, uint256 depth) = registry.root(address(coCreateEditions[i]), 1);
            assertEq(depth, i);
        }
        ...
        addParams.parentEdition = address(coCreateEditions[MAX_CHILDREN]);
    ...
    function test__AddToken_ExtensionIsSet() external {
        IRouxEdition coCreateEdition_ = _createEdition(users.creator_1);
        ...
        assertTrue(coCreateEdition_.isRegisteredExtension(tokenId_, address(mockExtension)));
    }
    ...
    function test__AddToken_ExtensionIsSet_WithMintParams() external {
        ...
        assertTrue(coCreateEdition_.isRegisteredExtension(tokenId_, address(mockExtension)));
        assertEq(IExtension(address(mockExtension)).price(address(coCreateEdition_), tokenId_), extPrice);
    }
}
